...
<head>
    ...
    <link rel="stylesheet" href="style/pages-style/user-profile-page.css">
        ...
        <div class="centered">
            <div class="profile-container">
                ...
                <div class="bottom-section">
                    ...
                    <input type="text" name="sum"/><input type="submit" value = "Пополнить"/>
                    </form>
                    <button class="profile-button" onclick = "controller?command=forward_change_pass&method=POST">ИЗМЕНИТЬ ПАРОЛЬ</button>
                    <button class="profile-button" onclick = "controller?command=get_cources_command&method=POST">КУРСЫ</button>
                    <c:if test="${ role !='admin'}">
                    <button class="profile-button" onclick = "controller?command=become_admin&method=POST">СТАТЬ АДМИНОМ</button>
                    </c:if>
                    <c:if test ="${role != 'teacher' }">
                    <button class="profile-button" onclick = "controller?command=become_teacherd&method=POST">СТАТЬ УЧИТЕЛЕМ</button>
                    </c:if>
                    <c:if test="${role!=student}">
                    <button class="profile-button" onclick = "controller?command=become_student&method=POST">СТАТЬ СТУДЕНОМ</button>
                    </c:if>
                    <button class="profile-button" onclick = "controller?command=get_cources_command&method=POST">ВЫЙТИ</button>
                </div>

                <hr class="short-line text-left">
            </div>
        </div>
    </div>
